...
namespace RemoveDetect
{
    public class model
    {
        private string FaceSamplesPath = "source//"; //这个是训练库文件夹 需要手动复制到项目根目录下
        private CascadeClassifier faceClassifier = new CascadeClassifier("haarcascade_frontalface_default.xml"); //这个文件也放根目录
        ...
        public TrainedFaceRecognizer SetTrainedFaceRecognizer(FaceRecognizerType type)
        {
            ...
            tfr.faceRecognizer.Save("test.xml");
            return tfr;
        }
        /// <summary>
        /// 获取制定图片，识别出的人脸矩形框
